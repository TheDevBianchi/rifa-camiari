      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            aria-modal='true'
            className='fixed left-1/2 top-1/2 w-full max-w-md z-50 sm:max-h-[90vh] sm:w-full sm:rounded-lg sm:translate-x-[-50%] sm:translate-y-[-50%] p-0'
            initial={{ opacity: 0, y: 100, x: '-50%' }}
            animate={{ opacity: 1, y: '-50%', x: '-50%' }}
            exit={{ opacity: 0, y: 100 }}>
            <ScrollArea className='max-h-[90vh] sm:max-h-[80vh] w-full rounded-lg'>
              <div className='relative bg-gradient-to-br from-black to-gray-900 rounded-lg p-6 shadow-xl border border-primary-500/20 min-h-[60vh]'>
                {/* Close button */}
                <button
                  onClick={() => onClose()}
